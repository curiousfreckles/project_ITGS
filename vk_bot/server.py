...
def main():
    ...
    keyboard2 = {
        'one_time': False,
        'buttons': [[get_button(label="Спbcr", color="positive"), get_button(label="Новая задача", color="negative"),
        get_button(label="Приступить сейчас", color="primary"),
        get_button(label="Справка", color="default")
        ]]
    }
    ...
    if data['type'] == 'confirmation':
        ...
        time.sleep(5)
        api.messages.send(access_token=token, user_id=str(user_id), message='Тык', keyboard=keyboard2)
